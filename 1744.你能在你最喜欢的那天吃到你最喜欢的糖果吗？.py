...
class Solution:
    def canEat(self, candiesCount: List[int], queries: List[List[int]]) -> List[bool]:
        ...
        for candy in candiesCount:
            candiesNow += candy
            candiesBefore.append(candiesNow)
        rs = []
        for query in queries:
            ...
            minday = 1
            maxday = query[2]
            minC = days * minday # 那一天之前，最少得吃
            maxC = (days+1) * maxday
            if maxC > candiesBefore[favoType] and minC < candiesBefore[favoType+1]:
                rs.append(True)
            else:
                rs.append(False)
        return rs
...
if __name__ == "__main__":
    s = Solution()
    rs = s.canEat([7,4,5,3,8], [[0,2,2],[4,2,4],[2,13,1000000000]])
    rs = s.canEat([5,2,6,4,1], [[3,1,2],[4,10,3],[3,10,100],[4,100,30],[1,3,1]])
    print(rs)