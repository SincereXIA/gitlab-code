...
class ThroneInheritance:
    ...
    def birth(self, parentName: str, childName: str) -> None:
        ...
        child = Person(childName, parent)
        print(parent.name, " has child: ", child.name)
    ...
    def getInheritanceOrder(self) -> List[str]:
        curOrder = []
        p = self.king
        for child in self.king.children:
            print(child.name)

        def presearch(p: Person):
            if p.alive:
                curOrder.append(p.name)
            for child in p.children:
                print(p.name, " search: ", child.name)
                presearch(child)
        ...
        # obj.death(name)
        # param_3 = obj.getInheritanceOrder()        # @lc code=end
# @lc code=end
